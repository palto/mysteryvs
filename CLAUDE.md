...
Zero-config works because `next.config.ts` detects the local dev server itself: at startup it checks whether port 1153 is open, and if so, sets `NEXT_PUBLIC_LIVEBLOCKS_BASE_URL` for the rest of the process — both server code (`process.env`) and the browser bundle (via the config's `env` field). `app/liveblocks/liveblocks.ts` then defaults its secret to `sk_localdev` whenever no real `LIVEBLOCKS_SECRET` is configured. This means plain `npm run dev` also auto-detects an already-running `npm run dev:liveblocks` in another terminal, not just `dev:local`.
...
If port 1153 isn't open, `LIVEBLOCKS_SECRET` is required as before (e.g. a real key from `npm run env:pull`) and the app connects to the Liveblocks cloud. The dev server (local mode) supports Storage, Presence, Yjs, access-token auth, and the `@liveblocks/node` REST API; Comments/Notifications/AI Copilots are not supported (their APIs return empty dummy data).
...
- `LIVEBLOCKS_SECRET`: Required for cloud mode. Set in `.env` file. Not needed for local dev server mode — `app/liveblocks/liveblocks.ts` defaults to `sk_localdev` whenever it's unset.
- `NEXT_PUBLIC_LIVEBLOCKS_BASE_URL`: Not normally set by hand. `next.config.ts` sets it automatically to `http://localhost:1153` whenever that port is open at startup.
