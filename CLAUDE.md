...
Instead of connecting to the Liveblocks cloud, `npm run dev:local` runs everything locally against the open-source [Liveblocks dev server](https://liveblocks.io/docs/tools/dev-server) — a single command, no Liveblocks account and **no environment variables to set**. Requires [Bun](https://bun.sh) to be installed (`curl -fsSL https://bun.sh/install | bash`): the `liveblocks` CLI shells out to Bun to run the dev server, unrelated to the rest of the app's Node/Next.js toolchain. Under the hood (`package.json`'s `dev:local` script) it uses `concurrently` to start `liveblocks dev` (port 1153) and, once `wait-on` confirms that port is open, `next dev`. `-k` kills both processes together on exit (e.g. Ctrl+C). Room data persists across restarts in the gitignored `.liveblocks/` directory as per-room SQLite databases.

Zero-config works because `next.config.ts` detects the local dev server itself: it probes `http://localhost:1153` at startup (checking the response for the dev server's own signature text, not just an open port, to avoid false positives) and, if found, sets `NEXT_PUBLIC_LIVEBLOCKS_BASE_URL` for the rest of the process — both server code (`process.env`) and the browser bundle (via the config's `env` field). `app/liveblocks/liveblocks.ts` then defaults its secret to `sk_localdev` whenever no real `LIVEBLOCKS_SECRET` is configured. This detection only ever runs when `LIVEBLOCKS_SECRET` isn't already set, so a real cloud connection is never silently overridden — and it means plain `npm run dev` also auto-detects an already-running `npm run dev:liveblocks` in another terminal, not just `dev:local`. A manually set `NEXT_PUBLIC_LIVEBLOCKS_BASE_URL` in `.env.local` still works as before and skips detection entirely.

The tournament room is seeded automatically: `instrumentation.ts` runs `seedLocalLiveblocksRoomIfNeeded()` (`app/liveblocks/seedLocalRoom.ts`) once when the Next.js server boots. It's a no-op unless `NEXT_PUBLIC_LIVEBLOCKS_BASE_URL` ends up set (i.e. it does nothing in cloud mode), and a no-op if the room already exists (checked via `getRoom` before writing anything, so it never re-touches or overwrites live data). Seeding exists because server components read room storage via the REST API and would 404 before any client has connected. Its starting data comes from `app/liveblocks/initialStorageData.ts`, the single source of truth also used by `initialStorage` in `app/Room.tsx`.

Cloud mode (a real `LIVEBLOCKS_SECRET` configured, e.g. via `npm run env:pull`) is unaffected by any of this — detection is skipped entirely and the real secret is always used. The dev server (local mode) supports Storage, Presence, Yjs, access-token auth, and the `@liveblocks/node` REST API; Comments/Notifications/AI Copilots are not supported (their APIs return empty dummy data).
...
- `LIVEBLOCKS_SECRET`: Required for cloud mode. Set in `.env` file. Not needed for local dev server mode — `app/liveblocks/liveblocks.ts` defaults to `sk_localdev` when it's unset.
- `NEXT_PUBLIC_LIVEBLOCKS_BASE_URL`: Optional. Points both the server SDK and browser client at a local Liveblocks dev server (e.g. `http://localhost:1153`). Normally unnecessary — `next.config.ts` sets this automatically when it detects a running local dev server and no `LIVEBLOCKS_SECRET` is configured. Set it by hand only to point at a non-default Liveblocks-compatible endpoint.
